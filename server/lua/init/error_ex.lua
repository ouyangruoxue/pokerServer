--[[
-- 定义mysql 的错误编号，用于系统连接mysql错误的定义
-- 在访问mysql错误的时候用户如果需要返回mysql的错误查询，主要在日志分析和开发过程中的操作
-- 服务器错误信息来自下述源文件：
...
-- 			由于更新很频繁，这些文件中可能包含这里未列出的额外错误消息。服务器错误信息来自下述源文件：
--]]

ZS_ERROR_CODE = {}
ZS_ERROR_CODE.REDIS_CRE_CLI_ERROR = "创建redis客户端错误!";
ZS_ERROR_CODE.REDIS_NO_DATA = "redis 没有查询到数据!";
ZS_ERROR_CODE.REDIS_SET_ERROR = "redis set 命令失败!";
ZS_ERROR_CODE.REDIS_PEXPIRE_ERROR = "redis pexpire 命令失败!";

ZS_ERROR_CODE.RE_SUCCESS = 1;
ZS_ERROR_CODE.RE_FAILED = 0;
-- 需要权限访问的错误
ZS_ERROR_CODE.RE_ACCEESS_ERR = 401;
--mysql 错误定义
local mysql_error={}

mysql_error.ER_DUP_ENTRY=1062;
